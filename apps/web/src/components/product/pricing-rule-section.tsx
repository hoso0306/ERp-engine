...
function VersionTable({ productId, versions }: { productId: string; versions: PricingRuleVersion[] }) {
  return (
              ...
              <TableRow key={v.id}>
                ...
                <TableCell>
                  <Button
                    ...
                    render={
                      <Link
                        href={`/products/${productId}/pricing-rule/versions/${v.id}`}
                      />
                    }
                  >
...
export function PricingRuleSection({ productId }: { productId: string }) {
  const router = useRouter();
  const [versions, setVersions] = useState<PricingRuleVersion[]>([]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-muted-foreground">Quy tắc báo giá</h3>
