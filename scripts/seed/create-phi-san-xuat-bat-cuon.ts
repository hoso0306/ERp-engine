/**
 * Tạo vật tư "PHÍ SẢN XUẤT BẠT CUỐN" (đơn vị m², giá mặc định 18.000đ) và
 * thêm vào định mức vật liệu (Material Requirement) của sản phẩm "Bạt Cuốn"
 * (SP000116), expression = "area" (area = chieurong * chieucao).
 *
 ...
 * đúng nguyên tắc Versioning: nhân bản version ACTIVE hiện tại (KHÔNG đụng
 * tới version DRAFT nào khác đang treo sẵn — trên Production sản phẩm này
 * có 1 bản DRAFT v3 để nguyên không sửa, theo yêu cầu 14/08/2026) thành
 * DRAFT mới, thêm Item vào bản DRAFT đó, rồi activate (archive bản ACTIVE
 * cũ, promote bản mới).
 *
 ...
 *     ../../scripts/seed/create-phi-san-xuat-bat-cuon.ts
 */
...
const MATERIAL_NAME = 'PHÍ SẢN XUẤT BẠT CUỐN';
const UNIT_NAME = 'm²';
...
async function main() {
  ...
  try {
    ...
    // 2) Thêm vào Material Requirement của sản phẩm — nhân bản từ đúng
    //    version ACTIVE hiện tại (không đụng tới DRAFT khác nếu có).
