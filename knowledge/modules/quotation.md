...
- Chiết khấu nhóm khách (%)
- Giảm thêm theo % (tuỳ chọn)
- Giảm thêm theo số tiền (tuỳ chọn)
- Lý do giảm giá
- Giá bán cuối
- Thành tiền
...
```text
systemPrice
    × (1 − groupDiscount%)           ← chiết khấu nhóm khách, tự động
    × (1 − additionalDiscountPercent%)  ← giảm thêm theo %, tuỳ chọn
    − additionalDiscountAmount          ← giảm thêm theo số tiền, tuỳ chọn
= finalPrice
```

Cho phép dùng đồng thời cả hai loại giảm thêm (% và số tiền).

`finalPrice` phải ≥ 0. Hệ thống không cho lưu nếu kết quả âm.

## Chiết khấu nhóm khách

Tự động lấy từ Customer Group tại thời điểm tạo báo giá.
...
Bắt buộc nhập lý do giảm vào `Lý do giảm giá`.
...
Không cho phép sửa trực tiếp `Giá bán cuối`.
