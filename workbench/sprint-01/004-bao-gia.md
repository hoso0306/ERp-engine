...
- [ ] `PricingEngine` service được thiết kế độc lập, không phụ thuộc vào Quotation.
- [ ] API/service nhận input (productId + parameters) → trả về `systemPrice`.
- [ ] Xử lý đúng trường hợp không có Pricing Rule Version ACTIVE → trả về lỗi rõ ràng.
- [ ] Có unit test cho các trường hợp: tính đúng giá, thiếu rule, rule không ACTIVE.
- [ ] Chưa cần UI hoàn chỉnh — chỉ cần sẵn sàng tích hợp vào Task 03.
- [ ] Build thành công.
...
- [ ] API hoạt động (Quotation + QuotationItem).
- [ ] UI hoạt động.
- [ ] Search, Filter, Pagination.
- [ ] Thêm / sửa QuotationItem: nhập thông số → `systemPrice` tự động tính từ Pricing Engine.
- [ ] `QuotationItemParameter` được lưu dưới dạng snapshot (không FK).
- [ ] Trường `expiryDate` (tuỳ chọn): UI hiển thị badge cảnh báo "Đã quá hạn" nếu `expiryDate < hôm nay`. Không chặn Approve dù đã quá hạn.
- [ ] Approved / Cancelled hiển thị readonly, không cho sửa.
- [ ] Không cho phép xoá QuotationItem nếu Quotation đang ở trạng thái Approved hoặc Cancelled — API phải trả lỗi rõ ràng.
- [ ] Build thành công.
...
- [ ] Group Discount tự động lấy từ Customer Group khi tạo báo giá.
- [ ] Giảm thêm (% và/hoặc số tiền) hoạt động, có thể dùng đồng thời.
- [ ] `finalPrice` không được âm — hệ thống báo lỗi nếu kết quả âm.
- [ ] Không cho phép sửa trực tiếp `finalPrice`.
- [ ] Lưu đầy đủ audit khi giảm thêm: lý do (`reason`), người thực hiện (`createdBy`), thời gian (`createdAt`), giá trị giảm theo % (`discountPercent`), giá trị giảm theo số tiền (`discountAmount`).
- [ ] Build thành công.
...
- [ ] `POST /quotations/:id/send` hoạt động: Draft → Sent, ghi Timeline.
- [ ] `POST /quotations/:id/cancel` hoạt động: huỷ từ bất kỳ trạng thái nào, bắt buộc nhập lý do, ghi Timeline.
- [ ] Không dùng `PATCH { status: "..." }`.
- [ ] Toàn bộ Action đi qua `QuotationWorkflowService`.
- [ ] Build thành công.
...
- [ ] `POST /quotations/:id/approve` đi qua `QuotationWorkflowService.approve()`.
- [ ] Snapshot hoạt động (Product, Pricing Rule Version, Material Requirement Version, giá bán, giá vốn, chiết khấu, Production Center).
- [ ] Sales Order sinh thành công.
- [ ] Order BOM sinh thành công.
- [ ] Production Orders sinh đúng theo `productionCenterId` của từng Product.
- [ ] `salesOrderId` được gán vào Quotation.
- [ ] Test case: Production Order sinh lỗi → Order + Snapshot đều bị rollback.
- [ ] Test case: Approve 2 lần cùng một báo giá → lần 2 bị reject (`salesOrderId IS NOT NULL`).
- [ ] Test case: Product không có Pricing Rule Version ACTIVE → Approve reject ở bước validate.
- [ ] Ghi Timeline (QuotationApproved) kèm salesOrderId và danh sách Production Order đã sinh.
- [ ] Build thành công.
...
- [ ] PDF đúng mẫu.
- [ ] Download hoạt động.
- [ ] Hiển thị đúng dữ liệu (thông tin khách hàng, danh sách sản phẩm, thông số, giá, chiết khấu, tổng tiền).
- [ ] Tên file PDF theo format: `BG000001 - Tên Công ty.pdf`.
- [ ] Build thành công.
