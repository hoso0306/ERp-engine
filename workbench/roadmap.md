...
* [x] 009 — FE Dashboard (sprint-03) — **✅ hoàn thành 10/07/2026** (`sprint-03/009-fe-dashboard.md`): trang chủ `/` tổng hợp KPI + drill-down từ `GET /dashboard/overview` (1 lần gọi duy nhất) — khối Kinh doanh (đơn gần đây), Sản xuất (tiến độ + xưởng bận nhất), Kho (top tiêu thụ + sắp hết/hết hàng), Công nợ (sắp đến hạn + top khách nợ), Hàng hoàn (lý do trả hàng + khách trả nhiều nhất — Task 05 ban đầu hoãn chờ 007, làm ngay sau khi 007 commit `54dceb9`), Cảnh báo (link tới khách hàng/vật tư/đơn hàng); mỗi khối tự ẩn khi field BE trả `null` theo quyền. **Live-test ẩn/hiện theo quyền hoàn tất 10/07/2026** (sau khi 010 xong, dùng Role/User tuỳ chỉnh tạo qua `/settings/roles`) — phát hiện + sửa 1 bug BE thật: `GET /dashboard/overview` không lọc `alerts` theo quyền (rò dữ liệu công nợ/kho cho user chỉ có `dashboard.view`), đã sửa `dashboard.controller.ts` (gộp logic lọc dùng chung với `GET /dashboard/alerts`).
* [x] 010 — FE Cài đặt + Người dùng (sprint-03) — **✅ hoàn thành 10/07/2026** (lập kế hoạch sớm hơn thứ tự gốc, không phụ thuộc 007/008/009). BE nhỏ: `GET /permissions` (catalog cho màn gán quyền), `resetPassword` qua `PATCH /users/:id`. FE: `/settings` (Công ty, Bộ số chứng từ, Cấu hình hệ thống theo tab động Dashboard/Notification/Document/Security/Backup), `/settings/users` (CRUD + cấp lại mật khẩu tạm), `/settings/roles` + `/settings/roles/[id]` (CRUD + gán quyền theo nhóm resource + Disable); bật 3 menu Cài đặt/Người dùng/Vai trò. Verify sống bằng Playwright thật — sửa Company Settings đối chiếu đúng qua trang in báo giá, tạo/reset mật khẩu User, tạo Role gán quyền đối chiếu đúng qua API, Disable Role thành công.
